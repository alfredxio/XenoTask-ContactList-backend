...
//add contact
app.post('/contacts/:emailx', async (req, res) => {
    try {
...
app.put('/contacts/:emailx/:contactId', async (req, res) => {
    try {
  ...
  app.delete('/contacts/:emailx/:id', async (req, res) => {
    try {
      ...
      const contactIndex = user.contacts.findIndex((contact) => contact._id.toString() === id);
      if (contactIndex === -1) {
        return res.status(404).json({ error: 'Contact not found' });
      }
